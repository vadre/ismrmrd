  ...
  : public Entity
{
    ...
    std::vector<uint32_t> data_count; /**< Expected data count for a stream */
...
public:
    ...
    // Sizes
    void resize(uint32_t num_samples, uint32_t active_channels = 1,
...
public:
    ...
    Image(uint32_t matrix_size_x = 0, uint32_t matrix_size_y = 1,
          uint32_t matrix_size_z = 1, uint32_t channels = 1);
